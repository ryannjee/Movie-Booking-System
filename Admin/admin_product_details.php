<?php
   ...
   if(isset($_POST["edit"]))
   {
      ...
      header("location:admin_product_details_edit.php");
   }
   ...
   if(isset($_POST["delete"]))
   {
      ...
      $Login-> deteleProduct($productID);
      echo "<script>
      alert('Delete successfully!!!');
      window.location.href = 'admin_product_details.php';
...
if(isset($_GET['page']) && is_numeric($_GET['page'])){
    $_SESSION['current_page6'] = $_GET['page'];
    $page = $_GET['page'];
} else if(isset($_SESSION['current_page6']) && is_numeric($_SESSION['current_page6'])) {
    $page = $_SESSION['current_page6'];
} else {
    $page = 1;
}
                           ...
                           <div class="page_title">
                              <div class="search">
                                 ...
                                 <form action="admin_product_details.php" method="POST">
                                       ...
                                       <button class="search-button" style="float: right; margin-right: 15px;" name="add"><a href="addProduct.php" style="color: white;">Add Product</a></button> 
                                    </form>                
                                 </div>
                              </div>
                           </div>
                              ...
                              <table class="table table-striped">
                                  <thead>
                                    <tr>
                                       ...
                                       <?php 
                                                ...
                                                {                                                    
                                                    ...
                                                    if ($filter == "productID") 
                                                    {
                                                        ...
                                                        $sql = "SELECT * FROM product WHERE productID Like '%$title%' LIMIT $offset, $records_per_page";
                                                            if ($title == "") 
                                                            {
                                                                $sql = "SELECT * FROM product LIMIT $offset, $records_per_page";
                                                            }
                                                    ...
                                                    {
                                                        // Add appropriate conditions to the query based on the filter and key values
                                                        $sql = "SELECT * FROM product WHERE productName LIKE '%$title%' LIMIT $offset, $records_per_page";
                                                            if ($title == "") 
                                                            {
                                                                $sql = "SELECT * FROM product  LIMIT $offset, $records_per_page";
                                                            }
                                       ...
                                       <tr>
                                          <form action="admin_product_details.php" method="post">
                                          <td> <img class="img-responsive rounded-circle" src="../Mainpage/assets/img/Product/<?php echo $productDetail["image"]?>" style="width:25%;"></td>                                        
                                             <td><?php echo $productDetail["productID"]?></td>
                                             <td><?php echo $productDetail["productName"]?></td>
                                             <td><?php echo $productDetail["price"]?></td>
                                             <input type="hidden" value="<?php echo $productDetail["productID"]?>" name="productID">      
